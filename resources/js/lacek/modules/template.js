...
export default class Template {
    ...
    _uiHandleDarkMode(mode = 'init') {
        ...
        if (mode === 'init' && !self._lPage.classList.contains('dark-mode')) {
            if (self._lPage.classList.contains('sidebar-dark')) {
                localStorage.setItem('lacekDefaultsSidebarDark', true);
            } else {
                localStorage.removeItem('lacekDefaultsSidebarDark');
            }

            if (self._lPage.classList.contains('page-header-dark')) {
                localStorage.setItem('lacekDefaultsPageHeaderDark', true);
            } else {
                localStorage.removeItem('lacekDefaultsPageHeaderDark');
            }
        }

        // If remember-theme class is added in #page-container
        if (self._lPage.classList.contains('remember-theme')) {
            let darkMode = localStorage.getItem('lacekDarkMode') || false;
            ...
            if (mode === 'init') {
                ...
                localStorage.setItem('lacekDarkMode', true);
            } else if (mode === 'off') {
                localStorage.removeItem('lacekDarkMode');
            }
        } else if (mode === 'init') {
            localStorage.removeItem('lacekDarkMode');
        }
    ...
    _uiHandleTheme() {
        ...
        if (rememberTheme) {
            let themeName = localStorage.getItem('lacekThemeName') || false;
            ...
            localStorage.removeItem('lacekThemeName');
        }
        ...
        document.querySelectorAll('[data-toggle="theme"]').forEach(el => {
            el.addEventListener('click', e => {
                ...
                if (rememberTheme) {
                    localStorage.setItem('lacekThemeName', themeName);
                }
            });
        });
    }
    ...
    _uiApiLayout(mode = 'init') {
        ...
        let layoutAPI = {
            ...
            dark_mode_off: () => {
                if (!localStorage.getItem('lacekDefaultsSidebarDark')) {
                    self._lPage.classList.remove('sidebar-dark');
                }

                if (!localStorage.getItem('lacekDefaultsPageHeaderDark')) {
                    self._lPage.classList.remove('page-header-dark');
                }
